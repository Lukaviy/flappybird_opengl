...
RankTableScene_t::RankTableScene_t(RankTable_t& rank_table) : 
	...
	_view(sf::Rect<float>(0.f, 0.f, 500.f, 500.f)), _save_score_time(0), _place(0) {}
...
void RankTableScene_t::draw(sf::RenderTarget& target, sf::RenderStates states) const {
	...
	unsigned int end = std::min(_rank_table.size(), start + 10);
 	...
 	for (int i = start; i < end; i++) {
		...
		text.setFillColor(score_text_color);
		...
		text.move(100.f + std::min(0.5f, std::max(0.f, _elapsed_time - 0.5f)) * 2.f * 30.f, 0);
		target.draw(text, states);
	}
